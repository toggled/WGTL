...
from deeprobust.graph.defense_pyg import APPNP 
...
model = APPNP(nfeat=features.shape[1], nhid=32, nclass=int(labels.max())+1,\
              K=10, alpha=0.1, \
              dropout=args.drop_rate, lr=args.lr, with_bn=False, weight_decay=args.weight_decay, \
...
prefix2 = ["APPNP",'/content/drive/MyDrive/WitnesscomplexGNNs/APPNP'][args.colab]
os.system('mkdir -p '+prefix2)
csv_name = prefix2+'/APPNP_acc_'+args.dataset + "_" + str(args.ptb_rate) + '.'+attack+'.csv'
