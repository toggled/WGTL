...
model = SAGE(nfeat=features.shape[1], nhid=32, nlayers = 2, \
             nclass=int(labels.max())+1, \
                dropout=args.drop_rate, lr=args.lr, weight_decay=args.weight_decay, \
                    with_bn = True, device=device)
...
# os.system('mkdir -p SAGE/')
# csv_name = 'SAGE/SAGE_acc_'+args.dataset + "_" + str(args.ptb_rate) + '.'+attack+'.csv'
prefix2 = ["SAGE",'/content/drive/MyDrive/WitnesscomplexGNNs/SAGE'][args.colab]
os.system('mkdir -p '+prefix2)
csv_name = prefix2+'/SAGE_acc_'+args.dataset + "_" + str(args.ptb_rate) + '.'+attack+'.csv'
