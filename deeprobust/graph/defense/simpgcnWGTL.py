...
class SimPGCNWGTL(nn.Module):
    ...
    #         with_bias=True, \
    #         alpha = 0.8, beta = 0.1, gamma2 = 0.1, lambda_coeff = 0.001, aggregation_method = 'attention',
    #         seed = 1,
    #         device='None'):
    #     super(SimPGCNWGTL, self).__init__()

    #     assert device is not None, "Please specify 'device'!"
    #     self.knn_dirname = 'saved_knn'+str(seed)
    #     self.device = device
    #     self.nfeat = nfeat
    ...
    #     # self.global_linear_layer = torch.nn.Linear(50 * 50, nhid)
    #     # self.lcnn = ViT(
    #     #         image_size = 50,
    #     #         patch_size = 25,
    #     #         num_classes = nhid,
    ...
    def __init__(self, nnodes, nfeat, nhid, nclass, dropout=0.5, lr=0.01,
        ...
        self.attention = Attention(nhid)
        # self.topological_weights = nn.ParameterList()
        # self.topological_weights.append(Parameter(torch.randn(50, 50))) # learnable weight
        # for i in range(1):
        #     self.topological_weights.append(Parameter(torch.randn(50, 50))) # learnable weight
        # self.topolinear = nn.Linear(50, nhid)
    ...
    def get_knn_graph(self, features, k=20):
        if not os.path.exists(self.knn_dirname+'/'):
           os.mkdir(self.knn_dirname)
        if not os.path.exists(self.knn_dirname+'/knn_graph_{}.npz'.format(features.shape)):
            features[features!=0] = 1
            sims = cosine_similarity(features)
            np.save(self.knn_dirname+'/cosine_sims_{}.npy'.format(features.shape), sims)
            ...
            sp.save_npz(self.knn_dirname+'/knn_graph_{}.npz'.format(features.shape), adj_knn)
        else:
            print('loading '+self.knn_dirname+'/knn_graph_{}.npz...'.format(features.shape))
            adj_knn = sp.load_npz(self.knn_dirname+'/knn_graph_{}.npz'.format(features.shape))
    ...
    def initialize(self):
        ...
        # self.topolinear.reset_parameters() ####
    ...
    def fit(self, features, adj, global_witness_complex_feat, local_witness_complex_feat, labels, idx_train, idx_val=None, train_iters=200, initialize=True, verbose=False, normalize=True, patience=100, **kwargs):
        if initialize:
            self.initialize()
    ...
    #     emb, att = self.attention(x.unsqueeze(1))
    #     global_witness_comp_topo = global_witness_comp_topo.view(global_witness_comp_topo.size(1))
    #     # print('x.shape: ',x.shape)
    #     # print('after attention() emb.shape: ',emb.shape)
    #     # print('global_witness_comp_topo.shape: ',global_witness_comp_topo.shape)

    #     x = torch.einsum('nb, b-> nb', emb, global_witness_comp_topo)
    #     x = F.dropout(x, self.dropout, training=self.training)
    #     # emb = torch.stack([x, global_witness_comp_topo], dim=1) #
    #     # emb, att = self.attention(emb)#
    #     # x = emb #

    #     x = (s_o * self.gc2(x, adj) + (1-s_o) * self.gc2(x, adj_knn)) + (gamma) * Dk_o * self.gc2(x, self.identity)
    #     # print('x.shape: ',x.shape)
    #     # global_top = self.global_linear_layer(self.global_witness_complex_feat.view(1,-1)).view(1, -1, self.hidden_sizes[0])
    #     # print('global_top: ',global_top.shape)
    #     # x = x + self.beta * global_top #

    #     x = F.log_softmax(x, dim=1)
    #     self.ss = torch.cat((s_i.view(1,-1), s_o.view(1,-1), gamma*Dk_i.view(1,-1), gamma*Dk_o.view(1,-1)), dim=0)
    #     # return x, embedding, loss_topo
    ...
    def myforward(self, fea, adj):
        ...
        x = F.dropout(x, self.dropout, training=self.training)
        embedding = x.clone()
        # print('x.shape: ',x.shape, ' => ',self.global_witness_complex_feat.shape, ' * ',self.topological_weights[0].shape)
        # print('multiplication: ',(self.global_witness_complex_feat @ self.topological_weights[0]).shape)
        # proj = self.gamma2 * self.global_witness_complex_feat @ self.topological_weights[0]
        # linear_proj = self.topolinear(proj.squeeze(0))
        # print('linear_proj.shape: ',linear_proj.shape)
        # x = x @ linear_proj.T #########
        ...
        # print('after rips')
        loss_topo = topo_loss(PD = local_witness_comp_topo_pd, p = 2, q= 1)
        # x = self.alpha * x + self.beta * global_witness_comp_topo + self.gamma2 * local_witness_comp_topo
        global_witness_comp_topo = global_witness_comp_topo.view(global_witness_comp_topo.size(1))
        ...
        # Dk_o = (x @ self.D_k[-1] + self.D_bias[-1])
        # x = (s_o * self.gc2(x, adj) + (1-s_o) * self.gc2(x, adj_knn)) + (gamma) * Dk_o * self.gc2(x, self.identity)
        # # x = x + self.gamma2 * self.global_witness_complex_feat @ self.topological_weights[-1] ##########
    ...
    def regression_loss(self, embeddings):
        if self.pseudo_labels is None:
            agent = AttrSim(self.features.to_dense(),seed=self.knn_dirname)
    ...
    def _train_without_val(self, labels, idx_train, train_iters, verbose):
        ...
        for i in range(train_iters):
            self.train()
            optimizer.zero_grad()
            output, embeddings, loss_topo = self.myforward(self.features, self.adj_norm)
            loss_train = F.nll_loss(output[idx_train], labels[idx_train])
            loss_ssl = self.lambda_ * self.regression_loss(embeddings)
            loss_total = loss_train + loss_ssl + self.lambda_coeff * loss_topo
            loss_total.backward()
            optimizer.step()
    ...
    def _train_with_val(self, labels, idx_train, idx_val, train_iters, verbose):
        if verbose:
            print('=== training SimP-GCN+WGTL model ===')
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
    ...
    def _train_with_early_stopping(self, labels, idx_train, idx_val, train_iters, patience, verbose):
        if verbose:
            print('=== training SimP-GCN+WGTL model w/ early stopping ===')
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
...
class AttrSim:
    ...
    def __init__(self, features,seed):
        ...
        self.knn_dirname = seed 

    def get_label(self, k=5):
        features = self.features
        if not os.path.exists(self.knn_dirname+'/cosine_sims_{}.npy'.format(features.shape)):
            sims = cosine_similarity(features)
            np.save(self.knn_dirname+'/cosine_sims_{}.npy'.format(features.shape), sims)
        else:
            print('loading '+self.knn_dirname+'/cosine_sims_{}.npy'.format(features.shape))
            sims = np.load(self.knn_dirname+'/cosine_sims_{}.npy'.format(features.shape))

        if not os.path.exists(self.knn_dirname+'/attrsim_sampled_idx_{}.npy'.format(features.shape)):
            try:
            ...
            np.save(self.knn_dirname+'/attrsim_sampled_idx_{}.npy'.format(features.shape), sampled)
        else:
            print('loading '+self.knn_dirname+'/attrsim_sampled_idx_{}.npy'.format(features.shape))
            sampled = np.load(self.knn_dirname+'/attrsim_sampled_idx_{}.npy'.format(features.shape))
