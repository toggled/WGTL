...
class SAGEConv(MessagePassing):
    def __init__(self, in_channels,
                 out_channels, with_bias=True, **kwargs):  # yapf: disable
        ...
        #self.lin_l = Linear(in_channels[0], out_channels, bias=with_bias)
    ...
    def forward(self, x, adj):
        ...
        if self.edge_index is None:
            self.edge_index = torch.LongTensor((adj.to_dense()).nonzero().cpu().numpy()).to(x.get_device())
        edge_index = self.edge_index
        out = self.propagate(edge_index.transpose(0,1), x=x.to_dense(), size=None)
        out = self.lin_l(out)
...
class SAGEWGTL(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,
        ...
        self.gat1 = SAGEConv(nfeat, nhid, with_bias=with_bias)
        self.gat2 = SAGEConv(nfeat, nhid, with_bias=with_bias)
    ...
    def forward(self, x, adj, global_witness_complex_feat, local_witness_complex_feat):
        ...
        sim_matrix = cosine_similarity(X=local_witness_comp_topo.detach().cpu().numpy(), Y=local_witness_comp_topo.detach().cpu().numpy())
    ...
    def _train_with_early_stopping(self, labels, idx_train, idx_val, train_iters, patience, verbose):
        if verbose:
            print('=== train w/ early stopping sage model ===')
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
