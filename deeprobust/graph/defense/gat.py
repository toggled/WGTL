...
class GAT(nn.Module):
    ...
    # def fit(self, pyg_data, train_iters=1000, initialize=True, verbose=False, patience=100, **kwargs):
        ...
        self.data = Data(x=features,edge_index = torch.LongTensor(adj.nonzero()),\
                         y=labels,train_mask=idx_train0)
        self.data = self.data.to(self.device)
        # By default, it is trained with early stopping on validation
        self.train_without_early_stopping(train_iters, patience, verbose)
    ...
    def train_without_early_stopping(self, train_iters, patience, verbose):
        """early stopping based on the validation loss
        """
        if verbose:
            print('=== training GAT model ===')
