...
from deeprobust.graph.defense_pyg import H2GCN 
...
parser.add_argument('--dataset', type=str, default='cora', choices=['cora','citeseer','polblogs','pubmed','fb100','snap-patents'], help='dataset')
...
# model = H2GCN(nfeat=features.shape[1], nhid=32, nlayers = 2, \
            #  nclass=int(labels.max())+1, dropout=args.drop_rate, lr=args.lr, weight_decay=args.weight_decay, device=device)
...
d = features.shape[1]
n = features.shape[0]
# model = H2GCN(d, hidden_channels, c, n, SparseTensor.from_scipy(perturbed_adj), \
#                 num_layers=2,  num_mlp_layers=1, \
#                 dropout=args.drop_rate, lr=args.lr, weight_decay=args.weight_decay,
#                 device=device)
model = H2GCN(d, hidden_channels, c, n, perturbed_adj, \
                num_layers=2,  num_mlp_layers=1, \
                dropout=args.drop_rate, lr=args.lr, weight_decay=args.weight_decay,
                device=device)
...
data = data.to(device)
model.fit(data, train_iters=args.epoch, verbose=True)
# # using validation to pick model
model.eval()
# You can use the inner function of model to test
acc = model.test()
output = {'seed':args.seed,'acc':acc}
os.system('mkdir -p H2GCN/')
csv_name = 'H2GCN/H2GCN_acc_'+args.dataset + "_" + str(args.ptb_rate) + '.'+attack+'.csv'
